...
{
    static class Var
    {//配列の宣言で使うことが多い 不変普遍定数ではない　ユーザが勝手に変えろ
    ...
    private static double GetMangaTextConst(bool StandardModeIsChecked)
    {//図表がマンガ 小説がText それぞれ画像密度が違うので 閾値を変更したい、
    ...
    public static void CarmineCliAuto(in string PathName)
    {//ハフマンテーブルの最適化によってjpgサイズを縮小
    ...
    private static unsafe void WhiteCutColor(in string f, Mat q_img, ImageRect NewImageRect)
    {//カラー画像は階調値線形変換はしない
